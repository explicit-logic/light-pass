...
function AssessmentBar({ block, correction, currentSlug }: Props) {
  ...
  const evaluate = useCallback(
    (newMark: MARK_TYPE) => async () => {
      ...
      try {
        const finalMark = await correctionEvaluate({
      ...
      <div className="inline-flex border-b border-gray-200 dark:border-gray-700" role="group">
        <button
          ...
          className={`inline-flex items-center px-3 py-2 text-sm font-medium ${
            !verified || points !== MARKS.WRONG ? 'hover:bg-gray-400 dark:hover:bg-gray-700' : ''
          } text-gray-900 bg-white border-r border-gray-200 rounded-tl-lg focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:focus:ring-blue-500`}
          disabled={verified && points === MARKS.WRONG}
        ...
        <button
          ...
          className={`inline-flex items-center px-3 py-2 text-sm font-medium text-gray-900 bg-white focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 ${
            !verified || points !== MARKS.HALF ? 'hover:bg-gray-400 dark:hover:bg-gray-700' : ''
          } dark:bg-gray-800 dark:border-gray-700 dark:focus:ring-blue-500`}
          disabled={verified && points === MARKS.HALF}
        ...
        <button
          type="button"
          className={`inline-flex items-center px-3 py-2 text-sm font-medium text-gray-900 bg-white border-l border-gray-200 rounded-tr-lg ${
            !verified || points !== MARKS.RIGHT ? 'hover:bg-gray-400 dark:hover:bg-gray-700' : ''
          ...
          disabled={verified && points === MARKS.RIGHT}
