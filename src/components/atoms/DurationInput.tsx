...
export default class DurationInput extends React.Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props): void {
    if (this.props.value !== prevProps.value) {
      const [validatedTime] = validateTimeAndCursor(this.state._showSeconds, this.props.value, this.state._defaultValue, this.state._colon);
      this.setState({
        value: validatedTime,
      });
    }
  ...
  onInputChange(event: ChangeEvent<HTMLInputElement>, callback: onChangeType): void {
    ...
    const [validatedTime, validatedCursorPosition] = validateTimeAndCursor(this.state._showSeconds, newValue, oldValue, colon, newPosition);
  ...
  render(): ReactElement {
    ...
    const { onChange, showSeconds, input, inputRef, colon, ...props } = this.props; //eslint-disable-line no-unused-vars
    const onChangeHandler = (event: ChangeEvent<HTMLInputElement>) => this.onInputChange(event, (v: string) => onChange?.(v));

    return <input type="text" {...props} ref={inputRef} value={value} onChange={onChangeHandler} />;
  }
}
