...
((global, factory) => {
  ...
  jQuery.each('Boolean Number String Function Array Date RegExp Object Error Symbol'.split(' '), (_i, name) => {
  ...
  (() => {
    var i,
      ...
      unloadHandler = () => 
        setDocument();,
      inDisabledFieldset = addCombinator((elem) => elem.disabled === true && nodeName(elem, 'fieldset'), 
        dir: 'parentNode',
        next: 'legend',);
    ...
    Expr = jQuery.expr = {
      ...
      pseudos: {
        ...
        focus: (elem) => elem === safeActiveElement() && document.hasFocus() && !!(elem.type || elem.href || ~elem.tabIndex),
  ...
  if (documentElement.getRootNode) {
    isAttached = (elem) => jQuery.contains(elem.ownerDocument, elem) || elem.getRootNode(composed) === elem.ownerDocument;
  }
    ...
    if (computed) {
      return (
        (Number.parseFloat(curCSS(elem, 'marginLeft')) ||
          elem.getBoundingClientRect().left - swap(elem, { marginLeft: 0 }, () => elem.getBoundingClientRect().left)) + 'px'
      );
    }
  });
  ...
  jQuery.parseXML = (data) => {
    ...
    if (!xml || parserErrorElem) {
      jQuery.error('Invalid XML: ' + (parserErrorElem ? jQuery.map(parserErrorElem.childNodes, (el) => el.textContent).join('\n') : data));
    }
    return xml;
  };
  ...
  jQuery.each(['ajaxStart', 'ajaxStop', 'ajaxComplete', 'ajaxError', 'ajaxSuccess', 'ajaxSend'], (_i, type) => {
  ...
  jQuery.trim = (text) => (text == null ? '' : (text + '').replace(rtrim, '$1'));
