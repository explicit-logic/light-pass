...
((factory) => {
          ...
          if (!$(event.target).closest('.ui-menu-item').length) {
            this._delay(function () {
              this.document.one('mousedown', (event) => {
                if (event.target !== this.element[0] && event.target !== menuElement && !$.contains(menuElement, event.target)) {
                  this.close();
                }
              });
            });
  ...
  $.widget('ui.autocomplete', $.ui.autocomplete, {
    options: {
      messages: {
        ...
        results: (amount) => amount + (amount > 1 ? ' results are' : ' result is') + ' available, use up and down arrow keys to navigate.',
      },
    },
  ...
  $.extend(Datepicker.prototype, {
    ...
    _shouldFocusInput: (inst) => inst.input && inst.input.is(':visible') && !inst.input.is(':disabled') && !inst.input.is(':focus'),
    ...
    _selectMonthYear: function (id, select, period) {
      ...
      inst['selected' + (period === 'M' ? 'Month' : 'Year')] = inst['draw' + (period === 'M' ? 'Month' : 'Year')] = Number.parseInt(
    ...
    _getFormatConfig: function (inst) {
      ...
      shortYearCutoff =
        typeof shortYearCutoff !== 'string' ? shortYearCutoff : (new Date().getFullYear() % 100) + Number.parseInt(shortYearCutoff, 10);
      return {
  ...
  $.widget('ui.draggable', $.ui.mouse, {
    ...
    _getRelativeOffset: function () {
      ...
      return {
        top: p.top - (Number.parseInt(this.helper.css('top'), 10) || 0) + (!scrollIsRootNode ? this.scrollParent.scrollTop() : 0),
        left: p.left - (Number.parseInt(this.helper.css('left'), 10) || 0) + (!scrollIsRootNode ? this.scrollParent.scrollLeft() : 0),
      };
    },
  ...
  $.widget('ui.resizable', $.ui.mouse, {
    ...
    _mouseStop: function (event) {
      ...
      if (this._helper) {
        ...
        left = Number.parseInt(this.element.css('left'), 10) + (this.position.left - this.originalPosition.left) || null;
        top = Number.parseInt(this.element.css('top'), 10) + (this.position.top - this.originalPosition.top) || null;
  ...
  $.ui.plugin.add('resizable', 'animate', {
    stop: function (event) {
      var that = $(this).resizable('instance'),
        ...
        left = Number.parseInt(that.element.css('left'), 10) + (that.position.left - that.originalPosition.left) || null,
        top = Number.parseInt(that.element.css('top'), 10) + (that.position.top - that.originalPosition.top) || null;
...
*/
  ...
  var effectScale = ($.effects.effect.scale = function (o, done) {
    ...
    var el = $(this),
      ...
      mode = $.effects.setMode(el, o.mode || 'effect'),
      percent = Number.parseInt(o.percent, 10) || (Number.parseInt(o.percent, 10) === 0 ? 0 : mode === 'hide' ? 0 : 100),
  ...
  var spinner = $.widget('ui.spinner', {
    ...
    _uiSpinnerHtml: () => "<span class='ui-spinner ui-widget ui-widget-content ui-corner-all'></span>",
  ...
  var tabs = $.widget('ui.tabs', {
    ...
    _createPanel: (id) =>
      $('<div>').attr('id', id).addClass('ui-tabs-panel ui-widget-content ui-corner-bottom').data('ui-tabs-destroy', true),
    ...
    _ajaxSettings: function (anchor, event, eventData) {
      return {
        ...
        beforeSend: (jqXHR, settings) => this._trigger('beforeLoad', event, $.extend({ jqXHR: jqXHR, ajaxSettings: settings }, eventData)),
      };
    },
